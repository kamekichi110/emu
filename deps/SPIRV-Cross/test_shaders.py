...
def get_shader_stats(shader):
   ...
   p = subprocess.Popen(['malisc', get_shader_type(shader), '--core', 'Mali-T760', '-V', shader],
...
def validate_shader_msl(shader, opt):
   ...
   try:
      ...
      subprocess.check_call(
         ['xcrun', '--sdk', msl_os, 'metal', '-x', 'metal', path_to_msl_standard(msl_path), '-Werror',
          '-Wno-unused-variable', msl_path])
...
def cross_compile_msl(shader, spirv, opt, iterations, paths):
   ...
   msl_args = [spirv_cross_path, '--entry', 'main', '--output', msl_path, spirv_path, '--msl', '--iterations',
               str(iterations)]
...
def cross_compile_hlsl(shader, spirv, opt, force_no_external_validation, iterations, paths):
   ...
   hlsl_args = [spirv_cross_path, '--entry', 'main', '--output', hlsl_path, spirv_path, '--hlsl-enable-compat',
                '--hlsl', '--shader-model', sm, '--iterations', str(iterations)]
   if '.line.' in shader:
...
def cross_compile_reflect(shader, spirv, opt, iterations, paths):
   ...
   sm = shader_to_sm(shader)
   subprocess.check_call(
      [spirv_cross_path, '--entry', 'main', '--output', reflect_path, spirv_path, '--reflect', '--iterations',
       str(iterations)])
...
def cross_compile(shader, vulkan, spirv, invalid_spirv, eliminate, is_legacy, flatten_ubo, sso, flatten_dim, opt,
   ...
   if vulkan or spirv:
      subprocess.check_call([spirv_cross_path, '--entry', 'main', '--vulkan-semantics', '--output', vulkan_glsl_path,
                             spirv_path] + extra_args)
...
def test_shader(stats, shader, args, paths):
   ...
   spirv, glsl, vulkan_glsl = cross_compile(joined_path, vulkan, is_spirv, invalid_spirv, eliminate, is_legacy,
                                            flatten_ubo, sso, flatten_dim, args.opt and (not noopt), push_ubo,
                                            args.iterations, paths)
...
def test_shader_hlsl(stats, shader, args, paths):
   ...
   is_spirv = shader_is_spirv(shader[1])
   noopt = shader_is_noopt(shader[1])
   spirv, hlsl = cross_compile_hlsl(joined_path, is_spirv, args.opt and (not noopt), args.force_no_external_validation,
                                    args.iterations, paths)
...
def test_shaders(backend, args):
   if args.malisc:
      with open('stats.csv', 'w') as stats:
         print(
            'Shader,OrigRegs,OrigUniRegs,OrigALUShort,OrigLSShort,OrigTEXShort,OrigALULong,OrigLSLong,OrigTEXLong,CrossRegs,CrossUniRegs,CrossALUShort,CrossLSShort,CrossTEXShort,CrossALULong,CrossLSLong,CrossTEXLong',
            file=stats)
...
def main():
   ...
   if (args.parallel and (args.malisc or args.force_no_external_validation or args.update)):
      sys.stderr.write(
         'Parallel execution is disabled when using the flags --update, --malisc or --force-no-external-validation\n')
      args.parallel = False
